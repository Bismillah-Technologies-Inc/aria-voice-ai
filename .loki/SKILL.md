...
**New in v5.0.0:** Multi-provider support (Claude/Codex/Gemini/Cline/Aider), abstract model tiers, degraded mode for non-Claude providers. See `.loki/skills/providers.md`.
...
**Degraded mode (Codex/Gemini/Cline/Aider):** No parallel agents or Task tool. Codex has MCP support. Runs RARV cycle sequentially. See `.loki/skills/model-selection.md`.

**Git worktree parallelism:** For true parallel feature development, use `--parallel` flag with run.sh. See `.loki/skills/parallel-workflows.md`.
...
- Load only 1-2 skill modules at a time (from .loki/skills/00-index.md)
...
| `.loki/skills/00-index.md` | Session start | Never |
...
```
1. Read .loki/skills/00-index.md (once per session)
...
| Quality gates 3-reviewer system | Implemented (v5.35.0) | 5 specialist reviewers in `.loki/skills/quality-gates.md`; execution in run.sh |
